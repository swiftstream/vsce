...
import { buildProdFolder, indexFile, LogLevel, print, Webber } from '../webber'
...
export class Firebase {
    ...
    private updateIsInstalled = async () => {
        const v = this.webber.bash.which('firebase')
    ...
    private execute = async (args: string[], cwd: string): Promise<BashResult> => {
        if (!this.binPath)
            this.binPath = await this.webber.bash.which('firebase')
        if (!this.binPath)
